...
<div class="w-full space-y-2" role="group" aria-label="Number pad">
  ...
  <div class="flex items-center gap-2">
    ...
    >
      <button
        class={[
          ...
          isPaused && "cursor-not-allowed opacity-40",
        ]}
    ...
    <button
      ...
      onclick={onUndo}
      disabled={undoDisabled || isPaused}
    ...
    <button
      ...
      onclick={onHint}
      disabled={hintsDisabled || isPaused}
      aria-label="Get hint"
      title="Hint"
    >
      <span class="text-lg leading-none">💡</span>
  ...
  <div class="grid grid-cols-5 gap-1">
    ...
    <button
      class={[
        "flex items-center justify-center rounded-md min-h-12 text-xl font-bold transition-all active:scale-95 focus:outline-none focus:ring-2 focus:ring-red-500 bg-red-100 text-red-600 hover:bg-red-200 dark:bg-red-900/40 dark:text-red-400 dark:hover:bg-red-800/50",
        isPaused && "cursor-not-allowed opacity-40",
      ]}
      onclick={onErase}
      disabled={isPaused}
  ...
  <div class="flex items-center gap-4 px-1">
    <label
      class={[
        "flex cursor-pointer items-center gap-2 text-sm text-neutral-700 select-none dark:text-neutral-300",
        isPaused && "cursor-not-allowed opacity-40",
      ]}
    >
      <input
    ...
    {#if onSubmitPuzzle}
      <button
        class={[
          "ml-auto rounded-md border border-neutral-300 dark:border-neutral-600 px-2.5 py-1 text-xs font-medium text-neutral-600 dark:text-neutral-400 transition-colors hover:bg-neutral-100 dark:hover:bg-neutral-700 active:scale-95 focus:outline-none focus:ring-2 focus:ring-blue-500",
          isPaused && "cursor-not-allowed opacity-40",
        ]}
